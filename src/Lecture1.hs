...
getText x
    | x == 1 = show x ++ " element"
    | otherwise = show x ++ " elements"

lowerAndGreater :: Int -> [Int] -> String
lowerAndGreater n list = show n ++ " is greater than " ++ getText (length (filter (< n) list)) ++ " and lower than " ++ getText (length (filter (>n) list))
